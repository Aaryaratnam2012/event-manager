...
const EventsPage = () => {
  ...
  useEffect(() => {
    const filterEvents = () => {
      if (eventType === "upcoming") {
        const newEve = events.filter(
          (e) => new Date(e.dateTime).getTime() > Date.now()
        );
        console.log(newEve);
  ...
  return (
    ...
    <br />
        {
          filteredEvent.length === 0? (<p className="text-center text-5xl font-mono">No Upcoming Events !</p>) : (<div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading
